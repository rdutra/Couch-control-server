...
!define APP_NAME "CouchControl"
!define APP_PUBLISHER "CouchControl"
!define APP_VERSION "0.1.0-mvp"
...
Section "CouchControl Agent and CLI" SecMain
  ...
  CreateDirectory "$SMPROGRAMS\CouchControl"
  CreateShortcut "$SMPROGRAMS\CouchControl\CouchControl Agent.lnk" "$INSTDIR\agent\CouchControl.Agent.exe" "" "$INSTDIR\agent\CouchControl.Agent.exe" 0
  CreateShortcut "$SMPROGRAMS\CouchControl\CouchControl CLI.lnk" "$SYSDIR\cmd.exe" '/k "$INSTDIR\cli\CouchControl.Cli.exe"' "$INSTDIR\cli\CouchControl.Cli.exe" 0
  CreateShortcut "$SMPROGRAMS\CouchControl\Uninstall CouchControl.lnk" "$INSTDIR\Uninstall.exe"
SectionEnd
...
Section "Uninstall"
  ...
  Delete "$SMPROGRAMS\CouchControl\CouchControl Agent.lnk"
  Delete "$SMPROGRAMS\CouchControl\CouchControl CLI.lnk"
  Delete "$SMPROGRAMS\CouchControl\Uninstall CouchControl.lnk"
  RMDir "$SMPROGRAMS\CouchControl"
